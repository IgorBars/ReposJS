...
alert("Start fetchPjhyett_zepochkaPromisov");

fetch('https://api.github.com/users')
  // Загружаем данные (массив объектов пользователей) в формате json
  .then(response => response.json())
  // Делаем запрос к GitHub, взяв из одного из объектов поле name ( =login )
  //.then(user => fetch(`https://api.github.com/users/${name}`))
  .then(user => fetch(`https://api.github.com/users/${user[3].login}`))
