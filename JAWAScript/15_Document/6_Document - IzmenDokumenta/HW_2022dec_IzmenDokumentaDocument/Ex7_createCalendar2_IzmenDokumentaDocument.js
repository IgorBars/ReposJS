/* JawaScript - Ex7_createCalendar2_IzmenDokumentaDocument         
...
'use strict';
alert("Start Ex7_createCalendar2_IzmenDokumentaDocument");


let cal = document.getElementById('cal');

function createCalendar(elem, year, month) {
    let mon = month - 1;                        // місяць JS: 0-11
    let months = ['січень', 'лютий', 'березень', 'квітень', 'травень', 'червень', 'липень', 'серпень', 'вересень', 'жовтень', 'листопад', 'грудень'];
    cal.innerHTML += months[mon];         // заголовок місяця
...
// цикл по поточним числам місяця (поки у поточному місяці mon)
    for (let k = 1; new Date(year, mon, k).getMonth() == mon; k++) {   

        let day = new Date(year, mon, k).getDay();  // 0 - 6  (поточний день тижня (відповідно числу місяця))
